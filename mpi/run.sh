...
OUTPUT_FILE="pso_execution_times_v2.csv"
...
if ! mpic++ -o pso main.cpp pso.cpp; then
...
echo "dimension,n_pop,n_proc,run_number,execution_time" > "$OUTPUT_FILE"
...
do
    ...
    do
        for proc in $(seq 1 $NUM_PROCS)
        do  
            for i in $(seq 1 $NUM_RUNS)
            do
                echo "Running PSO with dimension $dimension and population size $n_pop and processo $proc..."
                ...
                mpirun -np $proc ./pso $dimension $n_pop>/dev/null
                ...
                echo "$dimension,$n_pop,$proc,$i,$TIME_STR" >> "$OUTPUT_FILE"
            done
        done
    done
done
